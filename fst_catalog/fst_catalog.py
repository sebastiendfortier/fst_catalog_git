...
class FstCatalog:
    ...
    def catalog(self, num_proc : int = 8) -> None:
        ...
        :type num_proc: int, optional
        """
        grouping1 = ['nomvar', 'typvar', 'etiket', 'ni', 'nj', 'nk', 'grtyp', 'grid', 'level_unit']
        grouping2 = ['typvar', 'etiket', 'ni', 'nj', 'nk', 'grtyp', 'grid', 'level_unit']

        with Pool(min(num_proc,len(self._files))) as p:
            results = p.map(FstCatalog._get_fst_file_records_index, self._files)
            df = pd.concat(results, ignore_index=True)

            results = p.map(FstCatalog._aggregate_dataframe_columns, [group.reset_index(drop=True) for _, group in df.groupby(grouping1)])
            df = pd.concat(results, ignore_index=True)

            # results = p.map(self._get_record_crs_info, [group.reset_index(drop=True) for _, group in df.groupby(grouping2)])
            # crs_info_df = pd.concat(results, ignore_index=True).sort_values('nomvar').reset_index(drop=True)
            # display(crs_info_df)
    ...
    def _get_description_and_units_from_optdict(nomvar: str, root: ET.ElementTree) -> dict:
        ...
        nomvar_metvar = root.find(".//metvar[nomvar='{}']".format(nomvar))
